...
// ==================== OTP ROUTES ====================
Route::get('/otp/verify/{purpose}', [LoginController::class, 'showOtpForm'])->name('otp.verify.form');
...
Route::get('/debug/check-otp/{identifier}/{otp}', function ($identifier, $otp) {
    $otpRecord = App\Models\Otp::where('otp', $otp)->first();

    if (!$otpRecord) {
        return response()->json(['error' => 'OTP not found']);
    }

    $now = Carbon\Carbon::now('Asia/Jakarta');
    $expires = Carbon\Carbon::parse($otpRecord->expires_at);

    return response()->json([
        'otp_record' => [
            'id' => $otpRecord->id,
            'otp' => $otpRecord->otp,
            'identifier_in_db' => $otpRecord->identifier,
            'identifier_you_checked' => $identifier,
            'is_used' => $otpRecord->is_used,
            'created_at' => $otpRecord->created_at,
            'expires_at' => $otpRecord->expires_at,
        ],
        'current_time' => $now->format('Y-m-d H:i:s'),
        'is_expired' => $expires->lt($now),
        'identifier_match' => $otpRecord->identifier === $identifier
    ]);
});
...
Route::prefix('admin')
    ...
    ->group(function () {
        ...
        Route::put('/santri/{id}', [AdminController::class, 'santriUpdate'])->name(
            'santri.update');
