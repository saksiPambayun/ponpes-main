...
<div class="card" style="background: #fff; border-radius: 20px;">
    ...
    <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="mb-4">
                    ...
                    <span class="px-2 py-1 text-xs rounded-full <?php echo e($santri->status == 'diterima' ? 'bg-green-100 text-green-700' : ($santri->status == 'ditolak' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700')); ?>">
                ...
                <div class="mb-4">
                    ...
                    <span class="px-2 py-1 text-xs rounded-full <?php echo e($santri->acceptance_status == 'accepted' ? 'bg-green-100 text-green-700' : ($santri->acceptance_status == 'rejected' ? 'bg-red-100 text-red-700' : ($santri->acceptance_status == 'waiting_list' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'))); ?>">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="acceptance_status" class="w-full px-3 py-2 border rounded-lg" required>
                            <option value="pending" <?php echo e($santri->acceptance_status == 'pending' ? 'selected' : ''); ?>>-- Pilih Status --</option>
                            <option value="accepted" <?php echo e($santri->acceptance_status == 'accepted' ? 'selected' : ''); ?>>✅ Diterima</option>
                            <option value="rejected" <?php echo e($santri->acceptance_status == 'rejected' ? 'selected' : ''); ?>>❌ Ditolak</option>
                    ...
                    <div>
                        ...
                        <input type="date" name="announcement_date" class="w-full px-3 py-2 border rounded-lg" value="<?php echo e($santri->announcement_date ? date('Y-m-d', strtotime($santri->announcement_date)) : date('Y-m-d')); ?>">
                    </div>
                </div>
                <div class="mt-4">
                    <label class="block text-sm font-medium mb-2">Catatan (Opsional)</label>
                    <textarea name="acceptance_note" rows="3" class="w-full px-3 py-2 border rounded-lg" placeholder="Alasan penerimaan/penolakan..."><?php echo e($santri->acceptance_note); ?></textarea>
                </div>
        ...
        <div class="mt-6 pt-6 border-t flex justify-end space-x-3">
            <a href="<?php echo e(route('admin.santri.index')); ?>" class="px-4 py-2 border rounded-lg">Kembali</a>
...
<div id="rejectModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full">
        ...
        <form id="rejectForm" action="<?php echo e(route('admin.santri.reject', $santri->id)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <div class="mb-4">
