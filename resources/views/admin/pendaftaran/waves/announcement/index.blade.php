{{-- resources/views/admin/pendaftaran/announcement/index.blade.php --}}
@extends('admin.layout')

@section('title', 'Kelola Pengumuman')
@section('page-title', 'Kelola Pengumuman Pendaftaran')
        ...
        <div class="p-6">
            <form action="" method="POST" id="publishForm">
                    ...
                    <select name="wave_id" id="wave_select" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                        ...
                        @foreach($waves as $wave)
                            <option value="{{ $wave->id }}">
                                {{ $wave->name }} ({{ \Carbon\Carbon::parse($wave->start_date)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($wave->end_date)->format('d/m/Y') }})
                                @if($wave->is_active) - Aktif @endif
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <input type="date" name="publish_date" id="publish_date" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" value="{{ date('Y-m-d') }}" required>
                </div>
                <div class="mb-4">
        ...
        <table class="min-w-full divide-y">
            ...
            <tbody class="bg-white divide-y">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="font-medium">{{ $announcement['wave_name'] }}</div>
                    </td>
                    <td class="px-6 py-4 text-sm">{{ \Carbon\Carbon::parse($announcement['publish_date'])->format('d/m/Y H:i') }}</td>
                    <td class="px-6 py-4 text-sm">{{ Str::limit($announcement['title'], 50) }}</td>
                    <td class="px-6 py-4 text-sm">{{ $announcement['total_students'] }} orang</td>
                    <td class="px-6 py-4">
                        <div class="flex space-x-2">
                            <button onclick="viewAnnouncementDetail({{ json_encode($announcement) }})" class="text-blue-600 hover:text-blue-800" title="Lihat Detail">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button onclick="printSpecificAnnouncement({{ json_encode($announcement) }})" class="text-green-600 hover:text-green-800" title="Cetak">
