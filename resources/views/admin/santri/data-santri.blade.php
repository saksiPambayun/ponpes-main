...
@section('content')
    <div class="container mx-auto px-4">
        ...
        <div class="bg-white rounded-xl shadow-sm mb-6">
            ...
            <div class="p-6">
                <form method="GET" action="{{ route('admin.data-santri.index') }}"
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-blue-700">
                            ...
                            Filter aktif:
                            @if(request('angkatan'))
                                <span class="font-semibold">Angkatan {{ request('angkatan') }}</span>
                            @endif
                            @if(request('search'))
                                @if(request('angkatan')) , @endif
                                <span class="font-semibold">Pencarian: "{{ request('search') }}"</span>
                            @endif
                        </p>
                    </div>
                    <a href="{{ route('admin.data-santri.index') }}" class="text-blue-600 hover:text-blue-800 text-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2 py-1 text-xs rounded-full
                                                {{ $santri->jenis_kelamin == 'Laki-laki' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700' }}">
                                        {{ $santri->jenis_kelamin ?? '-' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-700">
                                        {{ $santri->angkatan ?? date('Y', strtotime($santri->created_at)) }}
                                    </span>
                                </td>
            ...
            <div class="px-6 py-4 border-t border-gray-200">
                {{ $santriTetap->appends(request()->query())->links() }}
            </div>
        </div>
    </div>
    ...
    <!-- Delete Forms -->
    @foreach($santriTetap as $santri)
        <form id="deleteForm{{ $santri->id }}" action="{{ route('admin.data-santri.destroy', $santri->id) }}" method="POST"
            style="display: none;">
            @csrf
            @method('DELETE')
        </form>
