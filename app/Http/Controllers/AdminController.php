...
class AdminController extends Controller
{
    ...
    public function santriIndex()
    {
        ...
        // Statistik untuk badge dan card
        $stats = [
            ...
            'total' => SantriRegistration::where('status', 'diterima')->count(),
            'laki_laki' => SantriRegistration::where('status', 'diterima')->where('jenis_kelamin', 'Laki-laki')->count(),
            'perempuan' => SantriRegistration::where('status', 'diterima')->where('jenis_kelamin', 'Perempuan')->count(),
        ];

        // Untuk filter gelombang
        $waves = \App\Models\RegistrationWave::all();

        // Untuk filter angkatan - ambil dari kolom angkatan jika ada, atau dari created_at
        $angkatanList = SantriRegistration::where('status', 'diterima')
            ->when(\Schema::hasColumn('santri_registrations', 'angkatan'), function ($query) {
                return $query->whereNotNull('angkatan')->select('angkatan')->distinct();
            }, function ($query) {
                return $query->selectRaw('YEAR(created_at) as angkatan')->distinct();
            })
  ...
  public function santriStore(Request $request)
{
    $validated = $request->validate([
        ...
        'kk'              => 'nullable|file|max:20480',
    ]);

    $data = $validated;

    // Set angkatan jika tidak diisi
    ...
    // ========== UPLOAD KK (BYPASS fileinfo) ==========
    if ($request->hasFile('kk')) {
        $file = $request->file('kk');
        $extension = $file->getClientOriginalExtension();
        $filename = time() . '_kk.' . $extension;
        
        $destinationPath = storage_path('app/public/santri/kk');
        if (!file_exists($destinationPath)) {
            mkdir($destinationPath, 0777, true);
        }
        $file->move($destinationPath, $filename);
        $data['kk'] = 'santri/kk/' . $filename;
    }

    // ========== UPLOAD FOTO (BYPASS fileinfo) ==========
    if ($request->hasFile('foto')) {
        $file = $request->file('foto');
        $extension = $file->getClientOriginalExtension();
        $filename = time() . '_foto.' . $extension;
        
        $destinationPath = storage_path('app/public/santri/foto');
        if (!file_exists($destinationPath)) {
            mkdir($destinationPath, 0777, true);
        }
        $file->move($destinationPath, $filename);
        $data['foto'] = 'santri/foto/' . $filename;
    }
...
if ($request->hasFile('foto')) {
    try {
        ...
        // Redirect berdasarkan route sebelumnya
        if (str_contains(url()->previous(), 'data-santri')) {
            return redirect()->route('admin.data-santri.index')->with('success', 'Data santri berhasil diupdate');
        }
    ...
    public function pegawaiStore(Request $request)
    {
        $validated = $request->validate([
            ...
            'alamat'            => 'nullable|string',
            'foto_ktp'    => 'nullable|file|max:40960',
            'foto_npwp'   => 'nullable|file|max:40960',
            'foto_ijazah' => 'nullable|file|max:40960',
        ]);
    ...
    public function skStore(Request $request)
    {
        $validated = $request->validate([
            ...
            'file_sk'    => 'nullable|file|max:5120',
        ]);
    ...
    public function aktaYayasanStore(Request $request)
    {
        $validated = $request->validate([
            ...
            'judul' => 'nullable|string|max:255',
            'deskripsi'   => 'nullable|string',
            'file_akta'   => 'required|file| max:5120',
        ]);
    ...
    public function aktaWakafStore(Request $request)
    {
        $validated = $request->validate([
            ...
            'judul' => 'nullable|string|max:255',
            'deskripsi'   => 'nullable|string',
            'file_sertifikat' => 'required| max:5120',
        ]);
...
{
    // Query untuk santri yang sudah diterima
    $query = SantriRegistration::where('status', 'diterima');

    // Filter berdasarkan angkatan (HANYA JIKA KOLOMNYA ADA)
    if ($request->filled('angkatan') && $request->angkatan != '' && Schema::hasColumn('santri_registrations', 'angkatan')) {
    ...
    if (Schema::hasColumn('santri_registrations', 'jenis_kelamin')) {
        $lakiLaki = SantriRegistration::where('status', 'diterima')->where('jenis_kelamin', 'Laki-laki')->count();
        $perempuan = SantriRegistration::where('status', 'diterima')->where('jenis_kelamin', 'Perempuan')->count();
    } else {
        $lakiLaki = 0;
        $perempuan = 0;
    ...
    // Daftar angkatan (HANYA JIKA KOLOMNYA ADA)
    if (Schema::hasColumn('santri_registrations', 'angkatan')) {
        ...
        $angkatanList = collect([]); // KOSONGKAN SEMENTARA
    }
